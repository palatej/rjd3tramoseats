...
p2r_spec_tramo<-function(pspec){
  ...
  t<-pspec$transform
  transform=list(fn=enum_extract(modelling.Transformation, t$transformation), fct=t$fct)
  a<-pspec$automodel
...
r2p_spec_tramo<-function(rspec){
  ...
  # TRANSFORM
  pspec$transform$transformation<-enum_of(modelling.Transformation, rspec$transform$fn, "FN")
